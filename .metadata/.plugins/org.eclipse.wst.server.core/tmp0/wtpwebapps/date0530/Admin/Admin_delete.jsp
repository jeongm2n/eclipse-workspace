...
<%
	...
	if(rs.next()){ //삭제할 아이디가 DB에 있으면 삭제 후 Admin 화면으로 돌아감
		...
		out.println("<script>alert('회원 삭제가 완료되었습니다.'); location.replace('Admin.jsp');</script>");
		
	}
	else{ //없으면 alert으로 경고를 주고 Admin 화면으로 돌아감
		out.println("<script>alert('해당 ID의 회원은 없습니다! 다시 확인해주세요.'); location.replace('Admin.jsp');</script>");
	}
%>
</body>
</html>