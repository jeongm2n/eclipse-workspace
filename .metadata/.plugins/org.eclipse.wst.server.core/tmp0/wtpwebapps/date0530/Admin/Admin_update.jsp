...
<form name="Updateform" action=Admin_updateOk.jsp method=get onSubmit="return validateUpdate();">
<table border=1>
	<tr><td class="front">아이디</td>
    <td class="rear"><input type="text" class="textbox" name="id" placeholder=<%=usr_id %>></td></tr>
    ...
    <td class="rear"><%=member_pwd%></td></tr>
   	...
   	<td class="rear">
   	<%
   		String radio = "<input type='radio' name='dept' value='";
   		String sql2 = "SELECT dept_no, dept_name FROM dept";
	   	PreparedStatement pstmt2 = conn.prepareStatement(sql2);
		ResultSet rs2 = pstmt2.executeQuery();
		
		while(rs2.next()){
			int dept = rs2.getInt("dept_no");
			String dept_name = rs2.getString("dept_name");
			if(dept_no == dept){
				out.println(radio+Integer.toString(dept)+"' checked='checked'/>"+dept_name);
			}
			else{
				out.println(radio+Integer.toString(dept)+"'/>"+dept_name);
			}
		}
   	%></td></tr>
