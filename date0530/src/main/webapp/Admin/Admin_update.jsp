...
<%
	...
	if(rs.next()){ //해당 id가 있다면 update 
		...
		out.println("<script>alert('"+update_id+"의 회원정보 수정이 완료되었습니다.'); location.replace('Admin.jsp');</script>");
	}
	else{ //해당 아이디가 없으면 update 안함
		out.println("<script>alert('해당 아이디가 없습니다. ID를 다시 확인해주세요'); location.replace('Admin.jsp');</script>");
	}
%>
</body>
</html>